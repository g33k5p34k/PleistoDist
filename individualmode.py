...
def individualmode(intervalfile, points):
    ...
    for i in intervalfile['Interval'].tolist(): #convert interval column into list and iterate through each interval
        if i == 0: #because Euclidean distance between points should only be calculated for the first interval, run interval0 separately
            ...
            arcpy.MakeFeatureLayer_management("output/shapefile/interval0.shp",intervalshp) #load island polygon for interval 0
            #open output files and write header row
            f = open("output/individual_euclidean_interval0.csv","w") #this output file is for the inter-point Euclidean distance
            g = open("output/individual_leastcost_interval0.csv","w") #this output file is for the least-cost path distance
            f.write(header)
            g.write(header)
            inraster = arcpy.ASCIIToRaster_conversion("output/raster/interval0.asc") #convert cost raster into a readable format
